...
use font::{FontHandleMethods, FontMetrics, FontTableMethods, GPOS, KERN};
    ...
    fn binary_search(&self, first_glyph: GlyphId, second_glyph: GlyphId) -> Option<i16> {
        ...
        while start < end {
            ...
            if key > query {
                ...
                return BigEndian::read_i16(&pairs[i * KERN_PAIR_LEN + 4..]);
            }
        }
    }
}
        ...
        match template.ctfont(size) {
            Some(ref ctfont) => {
                let mut handle = FontHandle {
                    ...
                    can_do_fast_kerning: false,
                };
                handle.h_kern_subtable = handle.find_h_kern_subtable();
                handle.can_do_fast_kerning = handle.h_kern_subtable.is_some() ||
    ...
    fn glyph_h_kerning(&self, first_glyph: GlyphId, second_glyph: GlyphId) -> FractionalPixel {
        if let Some(ref table) = self.h_kern_subtable {
            if let Some(font_units) = table.binary_search(first_glyph, second_glyph) {
                return font_units * table.pixels_per_font_unit;
            }
        }
        0.0
    }
