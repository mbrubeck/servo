...
pub struct BaseFlow {
    ...
    ///
    /// FIXME(mbrubeck): combine with block_container_inline_size and maybe
    /// block_container_explicit_block_size, to guarantee they are set at the same time?  Or just
    /// store a link to the containing block flow.
